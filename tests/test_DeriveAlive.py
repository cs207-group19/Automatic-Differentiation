...
def test_DeriveAlive_Vec():
	...
	def test_vector_input():
		...
		np.testing.assert_array_equal(div_mix.der, np.array([[0, 0], [-1 / 2, 0], [-1 / 3, 0], [0, 0]]))
		np.testing.assert_array_equal(div_mix_rev.val, div_mix.val)
		np.testing.assert_array_equal(div_mix_rev.der, np.array([[0, 0], [1 / 2, 0], [1 / 3, 0], [0, 0]]))
...
def test_DeriveAlive_vector_functions():
	...
	# Run tests within test_DeriveAlive_vector_functions()
	# test_neg()
	# test_abs()
	# test_constant()
	# test_add()
